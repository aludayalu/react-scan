...
const flattenTree = (
  ...
  return nodes.reduce<FlattenedNode[]>((acc, node, index) => {
    ...
    const renderData = node.fiber?.type
      ? getRenderData(node.fiber as Parameters<typeof getRenderData>[0])
      : undefined;
...
export const ComponentsTree = () => {
  ...
  const searchStateValue = useSignalValue(searchState);
...
`}
          ...
          >
            ...
            {searchStateValue.query ? (
              <>
                <span className="flex items-center gap-x-0.5 text-xs text-neutral-500">
                  {searchStateValue.currentMatchIndex + 1}
                  {'|'}
                  {searchStateValue.matches.length}
                </span>
                {!!searchStateValue.matches.length && (
                  <>
                    <button
