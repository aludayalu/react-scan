...
import { type ReadonlySignal, computed, useSignalValue } from '~web/utils/signals';
...
export const Content = () => {
  const headerClassNameValue = useSignalValue(headerClassName);
  return (
    <div
      className={cn(
...
const ContentView = ({ isOpen, children }: ContentViewProps) => {
  const isOpenValue = useSignalValue(isOpen);
  return (
    <div
      className={cn(
        'flex-1',
        ...
        'pointer-events-none',
        isOpenValue && 'opacity-100 delay-150 pointer-events-auto',
      )}
    >
