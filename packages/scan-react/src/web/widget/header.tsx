...
import { useSignalValue } from "~web/utils/signals";
import { HeaderInspect } from "~web/views/inspector/header";

export const Header = () => {
  const inspectState = useSignalValue(Store.inspectState);
  const widgetViews = useSignalValue(signalWidgetViews);
  const isInitialView = useDelayedValue(inspectState.kind === "focused", 150, 0);
  ...
  const isHeaderIsNotifications = widgetViews.view === "notifications";

  if (isHeaderIsNotifications) {
    return null;
  }

  const isFocused = inspectState.kind === "focused";
